...
def mlm_train_fn(cfg, train_loader, model, criterion, optimizer, epoch, scheduler, device):
    ...
    for step, item in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.apex):
            preds = model(item['input'], item['padding_mask'])
        
        loss = criterion(preds, item['output'])
        loss = torch.masked_select(loss.view(-1, 1), item['output'].view(-1, 1) == 1).mean()
...
def train_fn(cfg, train_loader, model, criterion, optimizer, epoch, scheduler, device):
    ...
    for step, item in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.apex):
            # y_preds = model(item['input'], item['padding_mask'])
            y_preds = model(item['input']) #[16, 29, 12]
...
def inference_fn(test_loader, model, device):
    ...
    for inputs in tk0:
        ...
        preds.append(torch.argmax(y_preds, dim=-1).cpu().numpy())
        probs.append(y_preds.softmax(dim=1)[:, 1].detach().cpu().numpy())
    predictions = np.concatenate(preds)
