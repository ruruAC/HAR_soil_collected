...
class TSTransformerEncoderMLM(nn.Module):

    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, dropout=0.1,
                 pos_encoding='fixed', activation='gelu', norm='BatchNorm', freeze=False):
        super(TSTransformerEncoderMLM, self).__init__()
...
class TSTransformerEncoderClassifier(nn.Module):
    ...
    def forward(self, X, padding_masks):
        ...
        # Output
        output = output[:, 0, :]  # zero-out padding embeddings
        # output = output.reshape(output.shape[0], -1)  # (batch_size, seq_length * d_model)
        output = self.output_layer(output)  # (batch_size, num_classes)
...
class TSTransformerEncoderGAPClassifier(nn.Module):
    ...
    def forward(self, X, padding_masks):
        ...
        # Output
        output = self.gap(output.transpose(1, 2)).squeeze()  # zero-out padding embeddings
        # output = output.reshape(output.shape[0], -1)  # (batch_size, seq_length * d_model)
        output = self.output_layer(output)  # (batch_size, num_classes)
...
class ConvSelfAttention(torch.nn.Module):
    def __init__(self, c_in, c_out=256, filter_map={1: 32, 3: 32, 5: 64, 7: 64, 9: 32, 11: 32}):
        ...
        self.wq = nn.ModuleList([Conv1dSame(c_in, co, ks=ks, stride=1) for ks, co in filter_map.items()])
    ...
    def forward(self, q, k, v, mask):  # q=k=v=x [b, seq_len, embedding_dim] embedding_dim其实也=d_model
        ...
        q = torch.cat([conv(q) for conv in self.wq], dim=1)  # =>[bs, d_model, seq_len] #batchsize,特征维度个数，词长度（一个时序点）[16, 256, 29]
...
class TSTransformerEncoderConvClassifier(nn.Module):
    ...
    def forward(self, X, padding_masks=None):
        ...
        inp = inp.permute(1, 0, 2)  # [seq_len, bs, d_model][29, 16, 256]
        inp = self.pos_enc(inp)     # add positional encoding
