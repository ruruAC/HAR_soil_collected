...
def load_UCR_UEA_dataset_from_tsfile(name, extract_path, split=None):
    ...
    if split in ['train', 'test']:
        split = split.upper()
        file_path = f'/home/zhangjunru/1104/datasets/Multivariate2018_ts/{name}//{name}_{split}.ts'
        X, y = load_from_tsfile_to_dataframe(file_path)
    else:
        X_train, y_train = load_from_tsfile_to_dataframe(f'/home/zhangjunru/1104/datasets/Multivariate2018_ts/{name}//{name}_TRAIN.ts')
        X_test, y_test = load_from_tsfile_to_dataframe(f'/home/zhangjunru/1104/datasets/Multivariate2018_ts/{name}//{name}_TEST.ts')
